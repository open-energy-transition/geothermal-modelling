...
def parse_inputs(base_path):
    ...
    eia_state_generation_reference_path = pathlib.Path(
        base_path, snakemake.input.eia_state_generation_path
    )
    ...
    eia_country_generation_reference = pd.read_csv(
        eia_country_generation_reference_path, index_col="Unnamed: 0"
    )
    eia_state_generation_reference = pd.read_excel(
        eia_state_generation_reference_path, sheet_name="Data"
    )

    return (
        network_pypsa_earth,
        eia_country_generation_reference,
        eia_state_generation_reference,
        gadm_shapes_path,
        extracted_time_resolution,
    )


def plot_state_by_state_comparison(
    pypsa_network,
    eia_reference,
    year_to_use,
    time_resolution,
    ...
    year_list = [
        val
        for val in list(eia_reference.columns)
        if val not in ["Data_Status", "State", "MSN"]
    ]
    ...
    eia_generation_reference_filtered = eia_reference[
        eia_reference["MSN"].isin(keywords_list)
    ][["State", "MSN", year_to_use]]
    eia_generation_reference_filtered = eia_generation_reference_filtered.rename(
        columns={"MSN": "carrier"}
    )
    eia_generation_reference_filtered.carrier = (
        eia_generation_reference_filtered.carrier.map(
            {
                "WYTCP": "onwind",
                "NUETP": "nuclear",
                "HYTCP": "hydro",
    ...
    eia_generation_reference_filtered[year_to_use] /= (
        1e3  # conversion from million kWh i.e., GWh to TWh
    )
    ...
    pypsa_network.generators = pypsa_network.generators.assign(
        gen_MWh=pypsa_network.generators_t.p.sum() * time_resolution
    )

    # PyPSA-Earth mapping the generators bus to the state names
    log_file.write(
        " --> Determine the PyPSA-Earth mapping the generators bus to the state names \n"
    )
    ...
    pypsa_generation_filtered = pypsa_network.generators.reset_index()[
        ["carrier", "bus", "gen_MWh"]
    ]
    pypsa_generation_filtered = pypsa_generation_filtered.rename(
        columns={"bus": "State"}
    )
    ...
    comparison_generation = comparison_generation.join(
        eia_generation_reference_filtered
    )
    comparison_generation = comparison_generation.rename(
        columns={"gen_MWh": pypsa_name, year_to_use: eia_name}
    )
    ...
    for car in ["nuclear", "onwind", "hydro", "geothermal"]:
        comparison_generation_filter = comparison_generation.reset_index().query(
            "carrier == @car"
        )
        comparison_generation_filter = comparison_generation_filter.set_index("State")[
            [pypsa_name, eia_name]
        ]
        fig = px.bar(comparison_generation_filter, barmode="group")
        fig.update_layout(title=f"Carrier = {car}", yaxis_title="Generation (TWh)")
        fig.write_html(
            pathlib.Path(plot_base_path, f"{car}_generation_comparison.html")
        )

    # compute error %s of the generation
    comparison_generation["error %"] = comparison_generation.apply(
        lambda x: (x[eia_name] - x[pypsa_name]) * 100 / x[eia_name], axis=1
    )


def plot_country_comparison(
    pypsa_network, eia_reference, year_to_use, time_resolution, log_file, plot_base_path
):
    """
    ...
    pypsa_network.storage_units = pypsa_network.storage_units.assign(
        p=pypsa_network.storage_units_t.p.sum() * time_resolution
    )
    pypsa_network.generators = pypsa_network.generators.assign(
        p=pypsa_network.generators_t.p.sum() * time_resolution
    )
    df_pypsa_generation = pd.concat(
        [
            pypsa_network.generators.groupby("carrier").p.sum(),
            pypsa_network.storage_units.groupby("carrier").p.sum(),
        ]
    )
    df_pypsa_generation.loc["wind"] = df_pypsa_generation.loc[
        ["offwind-ac", "offwind-dc", "onwind"]
    ].sum()
    df_pypsa_generation = df_pypsa_generation.drop(
        ["offwind-ac", "offwind-dc", "onwind"]
    )
    df_pypsa_generation /= 1e6
    df_pypsa_generation = df_pypsa_generation.round(2)
    df_pypsa_generation.name = pypsa_name
    df_eia_generation_year = eia_reference.loc[
    ...
    df_compare_generation = pd.concat(
        [df_pypsa_generation, df_eia_generation_year], axis=1
    )
    ...
    plt.savefig(
        pathlib.Path(plot_base_path, "countrywide_electricity_generation.png"), dpi=800
    )

    marginal_costs = pd.concat(
        [
            pypsa_network.generators.groupby("carrier")
            .marginal_cost.first()
            .sort_values(),
            pypsa_network.storage_units.groupby("carrier")
            .marginal_cost.first()
            .sort_values(),
        ]
    )
...
if __name__ == "__main__":
    ...
    (
        ...
        eia_country_generation_df,
        eia_state_generation_df,
        gadm_path,
        time_res,
    ...
    if snakemake.params.plot_state_by_state_comparison:
        plot_state_by_state_comparison(
            network_pypsa_earth_df,
            eia_state_generation_df,
            year_for_comparison,
            time_res,
