...
def plot_network_topology_comparison(
    pypsa_df, eia_df, voltage_class, pypsa_title, fig_name
):
    pypsa_df.lines["line_width"] = 0.0
    pypsa_df.lines.loc[pypsa_df.lines["v_nom_class"] == voltage_class, "line_width"] = (
        1.0
    )
    ...
    pypsa_df.plot(
        line_widths=pypsa_df.lines["line_width"],
        bus_sizes=0,
        line_colors="blue",
        ax=ax1,
    )
...
def plot_network_topology_intersection(pypsa_df, eia_df, voltage_class, fig_name):
    ...
    base_network_pe_volt_class = gpd.GeoDataFrame(
        base_network_pe_volt_class,
    ...
    intersection_geometries = base_network_pe_volt_class.intersection(
        eia_base_network_volt_class
    )
...
def plot_network_crossings(
    pypsa_df,
    eia_df,
    color_dictionary,
    voltage_classes_list,
    output_base_path,
    plot_base_path,
):
    ###############
    # GADM shapes #
    ###############
    gadm_eia_crossings_df = (
        eia_df.groupby(["state_0", "state_1", "v_nom_class"])
        .count()
        .reset_index()
        .loc[:, ("state_0", "state_1", "v_nom_class", "ID")]
        .rename(columns={"ID": "crossings"})
    )
    gadm_eia_crossings_df["source"] = "EIA"

    gadm_pearth_crossings_df = (
        pypsa_df.lines.groupby(["state_0", "state_1", "v_nom_class"])
        .count()
        .reset_index()
        .loc[:, ("state_0", "state_1", "v_nom_class", "Line")]
        .rename(columns={"Line": "crossings"})
    )
    gadm_pearth_crossings_df["source"] = "PyPSA"

    gadm_pearth_crossings_parallel_df = (
        pypsa_df.lines.groupby(["state_0", "state_1", "v_nom_class"])["num_parallel"]
        .sum()
        .reset_index()
        .loc[:, ("state_0", "state_1", "v_nom_class", "num_parallel")]
        .rename(columns={"num_parallel": "crossings"})
    )
    gadm_pearth_crossings_parallel_df["source"] = "PyPSA_parallel"

    gadm_network_counts = pd.concat(
        [
            gadm_eia_crossings_df,
            gadm_pearth_crossings_df,
            gadm_pearth_crossings_parallel_df,
    ...
    gadm_network_counts = (
        gadm_network_counts.set_index(["source", "state_0", "state_1", "v_nom_class"])
        .unstack("source")
        .droplevel(axis=1, level=0)
        .reset_index()
    )
    gadm_network_counts = gadm_network_counts.loc[
        :, ("state_0", "state_1", "v_nom_class", "PyPSA", "EIA", "PyPSA_parallel")
    ]
    ...
    state_crossings_counts_voltage = (
        state_crossings_counts.groupby("v_nom_class")[
            ["PyPSA", "EIA", "PyPSA_parallel"]
        ]
        .sum()
        .reindex(["Under 100", "100-161", "220-287", "345", "500", "735 And Above"])
        .reset_index()
    )
    state_crossings_counts_voltage.to_csv(
        pathlib.Path(output_base_path, "gadm_state_crossings_counts_by_voltage.csv"),
        index=False,
    )
    ...
    state_crossings_counts["delta_PyPSA"] = (
        (state_crossings_counts["PyPSA"] - state_crossings_counts["EIA"])
        / state_crossings_counts["EIA"]
        * 100.0
    )
    state_crossings_counts["delta_PyPSA_parallel"] = (
        (state_crossings_counts["PyPSA_parallel"] - state_crossings_counts["EIA"])
        / state_crossings_counts["EIA"]
        * 100.0
    )
    state_crossings_counts["coalesce"] = state_crossings_counts[
        ["state_0", "state_1"]
    ].agg("-->".join, axis=1)
    state_crossings_counts.to_csv(
        pathlib.Path(output_base_path, "gadm_state_crossings_counts.csv"), index=False
    )

    for voltage_class in voltage_classes_list:
        filtered_df = state_crossings_counts.loc[
            state_crossings_counts["v_nom_class"] == voltage_class
        ]
        fig = px.scatter(
            filtered_df,
            x="coalesce",
    ...
    state_lines_counts_by_voltage = (
        state_lines_counts.groupby("v_nom_class")[["PyPSA", "EIA", "PyPSA_parallel"]]
        .sum()
        .reindex(["Under 100", "100-161", "220-287", "345", "500", "735 And Above"])
        .reset_index()
    )
    state_lines_counts_by_voltage.to_csv(
        pathlib.Path(output_base_path, "gadm_state_lines_counts_by_voltage.csv"),
        index=False,
    )
    ...
    fig.write_image(
        pathlib.Path(plot_base_path, "gadm_state_lines_counts_by_voltage.png")
    )

    ##############
    # IPM shapes #
    ##############
    ipm_eia_crossings_df = (
        eia_df.groupby(["ipm_region_0", "ipm_region_1", "v_nom_class"])
        .count()
        .reset_index()
        .loc[:, ("ipm_region_0", "ipm_region_1", "v_nom_class", "ID")]
        .rename(columns={"ID": "crossings"})
    )
    ipm_eia_crossings_df["source"] = "EIA"

    ipm_pearth_crossings_df = (
        pypsa_df.lines.groupby(["ipm_region_0", "ipm_region_1", "v_nom_class"])
        .count()
        .reset_index()
        .loc[:, ("ipm_region_0", "ipm_region_1", "v_nom_class", "Line")]
        .rename(columns={"Line": "crossings"})
    )
    ipm_pearth_crossings_df["source"] = "PyPSA"

    ipm_pearth_crossings_parallel_df = (
        pypsa_df.lines.groupby(["ipm_region_0", "ipm_region_1", "v_nom_class"])[
            "num_parallel"
        ]
        .sum()
        .reset_index()
        .loc[:, ("ipm_region_0", "ipm_region_1", "v_nom_class", "num_parallel")]
        .rename(columns={"num_parallel": "crossings"})
    )
    ...
    ipm_network_counts = ipm_network_counts[
        [
            "ipm_region_0",
            "ipm_region_1",
            "v_nom_class",
    ...
    region_crossings_counts = ipm_network_counts.query(
        "ipm_region_0 != ipm_region_1"
    ).copy()

    region_crossings_counts_voltage = (
        region_crossings_counts.groupby("v_nom_class")[
            ["PyPSA", "EIA", "PyPSA_parallel"]
        ]
        .sum()
        .reindex(["Under 100", "100-161", "220-287", "345", "500", "735 And Above"])
    ...
    region_crossings_counts_voltage.to_csv(
        pathlib.Path(output_base_path, "ipm_region_crossings_counts_by_voltage.csv"),
        index=False,
    )
    ...
    fig.write_image(
        pathlib.Path(plot_base_path, "ipm_region_crossings_counts_by_voltage.png")
    )

    region_crossings_counts["delta_PyPSA"] = (
        (region_crossings_counts["PyPSA"] - region_crossings_counts["EIA"])
        / region_crossings_counts["EIA"]
        * 100.0
    )
    region_crossings_counts["delta_PyPSA_parallel"] = (
        (region_crossings_counts["PyPSA_parallel"] - region_crossings_counts["EIA"])
        / region_crossings_counts["EIA"]
        * 100.0
    )
    region_crossings_counts["coalesce"] = region_crossings_counts[
        ["ipm_region_0", "ipm_region_1"]
    ].agg("-->".join, axis=1)
    region_crossings_counts.to_csv(
        pathlib.Path(output_base_path, "ipm_region_crossings_counts.csv"), index=False
    )

    for voltage_class in voltage_classes_list:
        filtered_df = region_crossings_counts.loc[
            region_crossings_counts["v_nom_class"] == voltage_class
        ]
        fig = px.scatter(
            filtered_df,
            x="coalesce",
...
def plot_network_capacity_ipm(
    ...
    transmission_capacities_df[["ipm_region_0", "ipm_region_1"]] = np.sort(
        transmission_capacities_df[["ipm_region_0", "ipm_region_1"]]
    )
    grouped_df = transmission_capacities_df.groupby(["ipm_region_0", "ipm_region_1"])[
        "capacity (MW)"
    ].max()
    transmission_capacities_df = pd.merge(
        transmission_capacities_df, grouped_df, on=["ipm_region_0", "ipm_region_1"]
    )
    transmission_capacities_df = transmission_capacities_df.loc[
        :, ("ipm_region_0", "ipm_region_1", "capacity (MW)_y", "source")
    ].rename(columns={"capacity (MW)_y": "capacity (MW)"})
    transmission_capacities_df = transmission_capacities_df.drop_duplicates(
        keep="first"
    )

    pypsa_df["s_nom_num_parallel"] = pypsa_df["s_nom"] * pypsa_df["num_parallel"]
    pypsa_df["s_nom_num_parallel_pu"] = (
        pypsa_df["s_nom"] * pypsa_df["num_parallel"] * pypsa_df["s_max_pu"]
    )
    pypsa_df[["ipm_region_0", "ipm_region_1"]] = np.sort(
        pypsa_df[["ipm_region_0", "ipm_region_1"]]
    )
    ipm_region_pypsa_transmission_capacities = (
        pypsa_df.query("ipm_region_0 != ipm_region_1")
        .groupby(["ipm_region_0", "ipm_region_1"])["s_nom_num_parallel"]
        .sum()
        .reset_index()
        .loc[:, ("ipm_region_0", "ipm_region_1", "s_nom_num_parallel")]
        .rename(columns={"s_nom_num_parallel": "capacity (MW)"})
    )
    ...
    capacity_df = (
        capacity_df.set_index(["source", "ipm_region_0", "ipm_region_1"])
        .unstack("source")
        .droplevel(axis=1, level=0)
    )

    # remove values where IPM is zero or None
    capacity_df = capacity_df.query(
        "IPM != 0.0 & ~IPM.isna() & ~PyPSA.isna()"
    ).reset_index()

    # compute percentage error
    capacity_df["Error wrt IPM (%)"] = (
        (capacity_df["PyPSA"] - capacity_df["IPM"]) / capacity_df["IPM"] * 100.0
    )
    capacity_df["Error wrt PyPSA (%)"] = (
        (capacity_df["PyPSA"] - capacity_df["IPM"]) / capacity_df["PyPSA"] * 100.0
    )
    capacity_df["factor_IPM_over_PyPSA"] = capacity_df["IPM"] / capacity_df["PyPSA"]
    capacity_df["coalesce"] = capacity_df[["ipm_region_0", "ipm_region_1"]].agg(
        "-->".join, axis=1
    )
    ...
    fig.write_image(
        pathlib.Path(
            plot_base_path, f"{model}_box_transmission_capacities_wrt_PyPSA.png"
        )
    )
    ...
    capacity_df["geometry"] = capacity_df.apply(
        lambda x: spl.LineString([x.ipm_region_0_centroid, x.ipm_region_1_centroid]),
        axis=1,
    )
    ...
    ipm_geo_data_normal = ipm_geo_data_normal.explore(
        column="factor_IPM_over_PyPSA", cmap="jet", style_kwds={"weight": 5.0}
    )
    ipm_geo_data_normal.save(
        pathlib.Path(plot_base_path, f"{model}_interactive_map_line_normal.html")
    )
    ipm_map_line_outlier = ipm_geo_data_outlier.explore(
        column="factor_IPM_over_PyPSA", cmap="Reds_r", style_kwds={"weight": 5.0}
    )
    ipm_map_line_outlier.save(
        pathlib.Path(plot_base_path, f"{model}_interactive_map_line_outlier.html")
    )
...
def plot_network_capacity_reeds(
    ...
    transmission_capacities_df[["reeds_0", "reeds_1"]] = np.sort(
        transmission_capacities_df[["reeds_0", "reeds_1"]]
    )
    transmission_capacities_df = transmission_capacities_df.drop_duplicates(
        keep="first"
    )
    ...
    reeds_pypsa_transmission_capacities = (
        pypsa_df.query("reeds_0 != reeds_1")
        .groupby(["reeds_0", "reeds_1"])["s_nom_num_parallel"]
        .sum()
        .reset_index()
        .loc[:, ("reeds_0", "reeds_1", "s_nom_num_parallel")]
        .rename(columns={"s_nom_num_parallel": "Capacity (MW)"})
    )
    ...
    capacity_df = (
        capacity_df.set_index(["source", "reeds_0", "reeds_1"])
        .unstack("source")
        .droplevel(axis=1, level=0)
        .reset_index()
    )
    ...
    # remove values where IPM is zero or None
    capacity_df = capacity_df.query(
        "reeds != 0.0 & PyPSA != 0.0 & ~reeds.isna() & ~PyPSA.isna()"
    ).reset_index()

    # compute percentage error
    capacity_df["Error wrt reeds (%)"] = (
        (capacity_df["PyPSA"] - capacity_df["reeds"]) / capacity_df["reeds"] * 100.0
    )
    capacity_df["Error wrt PyPSA (%)"] = (
        (capacity_df["PyPSA"] - capacity_df["reeds"]) / capacity_df["PyPSA"] * 100.0
    )
    ...
    reeds_geo_data_normal = gpd.GeoDataFrame(
        capacity_df_normal, geometry=capacity_df_normal.geometry, crs="EPSG:3857"
    )
    reeds_geo_data_outlier = gpd.GeoDataFrame(
        capacity_df_outlier, geometry=capacity_df_outlier.geometry, crs="EPSG:3857"
    )
    reeds_geo_data_normal = reeds_geo_data_normal.explore(
        column="factor_reeds_over_PyPSA", cmap="jet", style_kwds={"weight": 5.0}
    )
    reeds_geo_data_normal.save(
        pathlib.Path(plot_base_path, f"{model}_reeds_interactive_map_line_normal.html")
    )
    reeds_map_line_outlier = reeds_geo_data_outlier.explore(
        column="factor_reeds_over_PyPSA", cmap="Reds_r", style_kwds={"weight": 5.0}
    )
    reeds_map_line_outlier.save(
        pathlib.Path(plot_base_path, f"{model}_reeds_interactive_map_line_outlier.html")
    )


def place_line_boundaries(
    lines_dataframe,
    gadm_dataframe,
    ipm_dataframe,
    reeds_dataframe,
    ...
    if network_used == "pypsa_earth":
        lines_dataframe_modified = gpd.GeoDataFrame(
            lines_dataframe,
            geometry=gpd.GeoSeries.from_wkt(lines_dataframe.bus_0_coors),
            crs="EPSG:4326",
        ).reset_index()
    else:
        lines_dataframe_modified = lines_dataframe.loc[
    ...
    spatial_join_gadm_sub_0 = (
        ...
        .loc[:, (id_column_name, "ISO_1")]
        .rename(columns={"ISO_1": "state_0"})
    )
    spatial_join_ipm_sub_0 = (
        lines_dataframe_modified.sjoin(ipm_dataframe, how="left")
        .loc[:, (id_column_name, "IPM_Region")]
        .rename(columns={"IPM_Region": "ipm_region_0"})
    )
    spatial_join_reeds_sub_0 = (
        lines_dataframe_modified.sjoin(reeds_dataframe, how="left")
        .loc[:, (id_column_name, "rb")]
        .rename(columns={"rb": "reeds_0"})
    )
    spatial_join_gadm_ipm_sub_0 = pd.merge(
        spatial_join_gadm_sub_0, spatial_join_ipm_sub_0, how="inner", on=id_column_name
    )
    spatial_join_sub_0 = pd.merge(
        spatial_join_gadm_ipm_sub_0,
        spatial_join_reeds_sub_0,
        how="inner",
        on=id_column_name,
    )
    ...
    if network_used == "pypsa_earth":
        lines_dataframe_modified = gpd.GeoDataFrame(
            lines_dataframe,
            geometry=gpd.GeoSeries.from_wkt(lines_dataframe.bus_1_coors),
            crs="EPSG:4326",
        ).reset_index()
    else:
        lines_dataframe_modified = lines_dataframe.loc[
    ...
    spatial_join_gadm_sub_1 = (
        lines_dataframe_modified.sjoin(gadm_dataframe, how="left")
        .loc[:, (id_column_name, "ISO_1")]
        .rename(columns={"ISO_1": "state_1"})
    )
    spatial_join_ipm_sub_1 = (
        lines_dataframe_modified.sjoin(ipm_dataframe, how="left")
        .loc[:, (id_column_name, "IPM_Region")]
        .rename(columns={"IPM_Region": "ipm_region_1"})
    )
    spatial_join_reeds_sub_1 = (
        lines_dataframe_modified.sjoin(reeds_dataframe, how="left")
        .loc[:, (id_column_name, "rb")]
        .rename(columns={"rb": "reeds_1"})
    )
    spatial_join_gadm_ipm_sub_1 = pd.merge(
        spatial_join_gadm_sub_1, spatial_join_ipm_sub_1, how="inner", on=id_column_name
    )
    spatial_join_sub_1 = pd.merge(
        spatial_join_gadm_ipm_sub_1,
        spatial_join_reeds_sub_1,
        how="inner",
        on=id_column_name,
    )
    ...
    lines_dataframe = pd.merge(
        lines_dataframe, spatial_join_sub_0, how="inner", on=id_column_name
    )
    lines_dataframe = pd.merge(
        lines_dataframe, spatial_join_sub_1, how="inner", on=id_column_name
    )
...
def parse_inputs(base_path, log_file):
    ...
    base_network_pypsa_earth_path = pathlib.Path(
        base_path, snakemake.input.base_network_pypsa_earth_path
    )
    base_network_pypsa_usa_path = pathlib.Path(
        base_path, snakemake.input.base_network_pypsa_usa_path
    )
    ...
    eia_base_network_path = pathlib.Path(
        base_path, snakemake.input.eia_base_network_path
    )
    ...
    base_network_pypsa_usa.loc[:, "Line"] = base_network_pypsa_usa.loc[
        :, "Line"
    ].astype(str)
    base_network_pypsa_usa.loc[:, "bus1"] = base_network_pypsa_usa.loc[
        :, "bus1"
    ].astype(str)
    ...
    lines_with_multilinestring_geometry = eia_base_network[
        eia_base_network["geometry_type"] == "MultiLineString"
    ]["OBJECTID_1"].values.tolist()
    multilinestring_ratio = (
        len(lines_with_multilinestring_geometry) / eia_base_network.shape[0] * 100.0
    )
    if multilinestring_ratio > 10.0:
        ...
        eia_base_network = eia_base_network[
    ...
    eia_base_network = place_line_boundaries(
        eia_base_network,
        gadm_shapes,
        ipm_shapes,
        reeds_shapes,
    ...
    eia_base_network = eia_base_network.rename(
        columns={"VOLTAGE": "v_nom", "VOLT_CLASS": "v_nom_class"}
    )
    ...
    lines_osm_raw = place_line_boundaries(
        lines_osm_raw,
        gadm_shapes,
        ipm_shapes,
        reeds_shapes,
    ...
    lines_osm_clean = place_line_boundaries(
        lines_osm_clean,
        gadm_shapes,
        ipm_shapes,
        reeds_shapes,
    ...
    base_network_pypsa_earth.lines["v_nom_class"] = base_network_pypsa_earth.lines[
        "v_nom"
    ]
    ...
    base_network_pypsa_earth.lines["v_nom_class"] = base_network_pypsa_earth.lines[
        "v_nom_class"
    ].replace(v_nom_class_dict_pypsa_earth)
    ...
    base_network_pypsa_usa = gpd.GeoDataFrame(
        base_network_pypsa_usa,
        geometry=gpd.GeoSeries.from_wkt(base_network_pypsa_usa["WKT_geometry"]),
        crs="EPSG:4326",
    ).reset_index()
    base_network_pypsa_usa[["sub_0_coors", "sub_1_coors"]] = (
    ...
    base_network_pypsa_usa["v_nom_class"] = base_network_pypsa_usa[
        "v_nom_class"
    ].replace(v_nom_class_dict_pypsa_usa)

    return (
        eia_base_network,
        base_network_pypsa_earth,
        base_network_pypsa_usa,
        lines_osm_raw,
        lines_osm_clean,
        ipm_shapes,
...
if __name__ == "__main__":
    ...
    output_path = pathlib.Path(
        default_path, "analysis", "outputs", "network_comparison"
    )
    ...
    ccs_color_dict = {
        "EIA": "#FF8C00",
        "PyPSA": "#0000FF",
        "PyPSA_parallel": "#228B22",
        "delta_PyPSA": "#0000FF",
        "delta_PyPSA_parallel": "#228B22",
        "Error (%)": "#FF7F50",
    }
    today_date = str(dt.datetime.now())
    log_output_file_path = pathlib.Path(
        log_path, f"output_network_comparison_{today_date[:10]}.txt"
    )
    ...
    network_eia_df.to_csv(
        pathlib.Path(output_path, "modified_eia_base.csv"), index=False
    )
    osm_lines_raw.to_csv(
        pathlib.Path(output_path, "modified_osm_lines_raw.csv"), index=False
    )
    osm_lines_clean.to_csv(
        pathlib.Path(output_path, "modified_osm_lines_clean.csv"), index=False
    )
    ...
    if snakemake.params.plot_network_topology:
        for selected_voltage_class in eia_voltage_classes:
            ...
            plot_network_topology_comparison(
                network_pypsa_earth_df,
                network_eia_df,
                selected_voltage_class,
                "PyPSA-Earth base network",
                fig_name_map,
            )
