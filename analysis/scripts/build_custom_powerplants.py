...
import pandas as pd
from _helpers_usa import config, eia_to_pypsa_terminology
import numpy as np
import swifter
...
def parse_inputs(base_path, log_file):
    ...
    eia_generators_df = pd.read_excel(eia_generators_path, sheet_name='Operable', skiprows=1)
...
def build_custom_pp(eia_generators_df, eia_plants_df, log_file):
    ...
    df_eia_reqd = eia_generators_df.loc[eia_generators_df.Technology.isin(eia_to_pypsa_dict.keys())]
    df_eia_reqd = df_eia_reqd[['Plant Name','Technology','Nameplate Capacity (MW)','Operating Year','Planned Retirement Year']]
    ...
    df_ppl = pd.DataFrame(
        ...
        'DamHeight_m','StorageCapacity_MWh','DateIn','DateRetrofit','DateMothball','DateOut','lat','lon','EIC',
        'projectID','bus']
    )
    ...
    df_ppl.loc[:,['lat','lon']] = df_ppl.swifter.apply(lambda x: eia_plants_df.query('`Plant Name` == @x["Name"]').iloc[0][['Latitude','Longitude']],axis=1).rename(columns={'Latitude':'lat','Longitude':'lon'})
...
def identify_ror_plants(custom_ppl_eia_df, custom_ppl_ror_df, log_file):
    ...
    custom_ppl_eia_df.loc[(custom_ppl_eia_df['Name'].isin(ror_plant_names)) & (custom_ppl_eia_df['Fueltype'] == "hydro"), 'Technology'] = "Run-Of-River"
...
if __name__ == '__main__':
    ...
    log_output_file_path = pathlib.Path(log_path, f"output_build_custom_powerplants_{today_date[:10]}.txt")
    log_output_file_path.touch(exist_ok=True)
    log_output_file = open(log_output_file_path, "w")

    eia_generators_df, eia_plants_df, ror_custom_ppl = parse_inputs(default_path, log_output_file)
    df_custom_ppl_eia = build_custom_pp(eia_generators_df, eia_plants_df, log_output_file)
    df_custom_ppl = identify_ror_plants(df_custom_ppl_eia, ror_custom_ppl, log_output_file)
