...
def parse_inputs(base_path):
    ...
    eia_state_temporal_installed_capacity_path = pathlib.Path(
        base_path, snakemake.input.eia_state_temporal_installed_capacity_path
    )
    eia_raw_reference_path = pathlib.Path(
        base_path, snakemake.input.eia_raw_reference_path
    )
    ...
    eia_installed_capacity_reference = pd.read_excel(
        eia_installed_capacity_reference_path, skiprows=1, index_col="Energy Source"
    )
    eia_state_temporal_installed_capacity_reference = pd.read_excel(
        eia_state_temporal_installed_capacity_path, skiprows=1
    )
    eia_raw_reference = pd.read_csv(eia_raw_reference_path)

    return (
        network_pypsa_earth,
        eia_installed_capacity_reference,
        eia_raw_reference,
        eia_state_temporal_installed_capacity_reference,
        gadm_shapes_path,
    )
...
def plot_capacity_spatial_representation(
    pypsa_network, plot_type, state_to_drop, plot_base_path, gadm_base_path
):
    iso_code_to_omit = []
    ...
    buses = pypsa_network.buses.query(
    ...
    pypsa_network.lines.loc[
        pypsa_network.lines.bus1.isin(iso_code_to_omit), "s_nom"
    ] = 0

    if plot_type == "capacity":
        cap = (
            pypsa_network.generators.query(
                "carrier != 'load' and bus in @b", local_dict={"b": bus_index}
            )
            .groupby(["bus", "carrier"])
            .p_nom.sum()
        )
        ...
        add_legend_patches(
            ax,
            colors=[
                pypsa_network.carriers.loc[carrier].color
                for carrier in carriers_with_capacity
            ],
        ...
        plt.savefig(
            pathlib.Path(
                plot_base_path, "installed_capacity_spatial_representation.png"
            ),
            dpi=800,
        )


def plot_capacity_state_by_state_comparison(
    pypsa_network,
    eia_reference,
    eia_raw_reference,
    year_to_use,
    ...
    eia_installed_capacity_by_state_year = eia_reference.loc[
        eia_reference["Year"] == year_to_use
    ]
    ...
    eia_installed_capacity_by_state_year = eia_installed_capacity_by_state_year.rename(
        columns=rename_cols
    )
    eia_installed_capacity_by_state_year = eia_installed_capacity_by_state_year.replace(
        {
            "carrier": {
                "Hydroelectric": "hydro",
    ...
    eia_installed_capacity_by_state_year = eia_installed_capacity_by_state_year.loc[
        eia_installed_capacity_by_state_year["Producer Type"]
    ...
    series_to_use = eia_installed_capacity_by_state_year.groupby(
        ["carrier", "state"]
    ).installed_capacity.sum()
    df = pd.DataFrame(
        {
            "carrier_state": series_to_use.index,
            "installed_capacity": series_to_use.values,
        }
    )
    df[["carrier", "state"]] = df["carrier_state"].apply(lambda x: pd.Series(x))
    eia_installed_capacity_by_state_year = df[
        ["carrier", "state", "installed_capacity"]
    ].copy()
    eia_installed_capacity_by_state_year.carrier = (
        eia_installed_capacity_by_state_year.carrier.str.lower()
    )

    eia_installed_capacity_by_state_year["carrier"] = (
        eia_installed_capacity_by_state_year["carrier"].apply(
            lambda x: rename_carrier(x)
        )
    )
    ...
    pypsa_installed_capacity_by_state = df[
        ["carrier", "GID_1_ACDC", "installed_capacity"]
    ].copy()
    pypsa_installed_capacity_by_state["GID_1"] = pypsa_installed_capacity_by_state[
        "GID_1_ACDC"
    ].str[:-3]

    gadm_gdp_usa_state = pd.DataFrame(
        get_gadm_mapping(gadm_shapes_path).items(), columns=["GID_1", "state"]
    )
    pypsa_df = pypsa_installed_capacity_by_state.merge(
        gadm_gdp_usa_state, left_on="GID_1", right_on="GID_1", how="inner"
    )
    ...
    pypsa_wind_df = pypsa_wind_df.groupby("state").agg(
        {"GID_1_ACDC": "first", "installed_capacity": "sum", "GID_1": "first"}
    )
    ...
    pypsa_gas_df = (
        pypsa_agg_df[pypsa_agg_df["carrier"].isin(["CCGT", "OCGT"])]
        .groupby("state")
        .agg({"GID_1_ACDC": "first", "installed_capacity": "sum", "GID_1": "first"})
    )
    ...
    df_full_merge = pd.DataFrame(
        eia_installed_capacity_by_state_year.set_index(["state", "carrier"])[
            "installed_capacity"
        ]
    )
    df_full_merge = df_full_merge.join(
        pypsa_agg_df.set_index(["state", "carrier"])["installed_capacity"],
        lsuffix="_eia",
        rsuffix="_pypsa",
    )
    df_full_merge /= 1000
    ...
    df_full_merge = df_full_merge.query(
        "carrier in @col", local_dict={"col": reqd_cols}
    )
    log_file.write("Merged the EIA and PyPSA installed capacity data state-wise \n")

    # Grouped bar plots to compare the installed capacities state-wise

    fig1 = px.bar(
        df_full_merge,
        y="installed_capacity_pypsa",
        color="carrier",
        barmode="stack",
        text_auto=".1f",
    )
    fig1.update_layout(
        yaxis_range=[0, 160], yaxis_title="Installed capacity PyPSA (GW)"
    )
    fig1.write_image(
        f"{plot_base_path}/installed_capacity_pypsa_statewise.png", scale=1.5
    )
    fig2 = px.bar(
        df_full_merge,
        y="installed_capacity_eia",
        color="carrier",
        barmode="stack",
        text_auto=".1f",
    )
    ...
    log_file.write(
        "Generated statewise installed capacity bar plots (colored by carrier) \n"
    )
    ...
    fig1.update_layout(
        yaxis_range=[0, 600], yaxis_title="Installed capacity PyPSA (GW)"
    )
    fig1.write_image(
        f"{plot_base_path}/installed_capacity_pypsa_carrierwise.png", scale=1.5
    )
    fig2 = px.bar(
        df_full_merge,
        y="installed_capacity_eia",
    ...
    fig2.write_image(
        f"{plot_base_path}/installed_capacity_eia_carrierwise.png", scale=1.5
    )
    log_file.write(
        "Generated carrier wise installed capacity bar plots (colored by state) \n"
    )
    ...
    df_ppl["carrier"] = df_ppl["carrier"].replace(
        {"onwind": "wind", "CCGT": "gas", "OCGT": "gas"}
    )
    ...
    df_compare = pd.DataFrame(
        df_full_merge.reset_index().set_index(["carrier", "state"])
    ).join(df_ppl_group)
    df_compare = df_compare.reset_index()
    df_compare["diff_pypsa"] = df_compare.apply(
    ...
    df_compare["diff_custom"] = df_compare.apply(
    ...
    tot_capacity = df_compare.sum()["installed_capacity_eia"]
    df_compare["error2_custom"] = df_compare.apply(
        lambda x: (x["installed_capacity_eia"] - x["Capacity"]) * 100 / tot_capacity,
        axis=1,
    )
    ...
    fig.update_layout(
        yaxis_range=[-160, 160], yaxis_title="Error % statewise", title="EIA vs PyPSA"
    )
    fig.write_image(
        f"{plot_base_path}/error_percent_statewise_eia_through_pypsa.png", scale=1.5
    )
    ...
    fig.update_layout(
        yaxis_range=[-160, 160],
        yaxis_title="Error % statewise",
        title="EIA vs raw custom powerplants",
    )
    fig.write_image(f"{plot_base_path}/error_percent_statewise_eia.png", scale=1.5)
    log_file.write(
        "Generated boxplots depicting statewise percentage errors in installed capacity \n"
    )
    ...
    log_file.write(
        "Generated boxplots depicting statewise difference in installed capacity in GW \n"
    )


def plot_capacity_country_comparison(
    pypsa_network, eia_reference, year_to_use, log_file, plot_base_path
):
    """
    ...
    df_pypsa_hydro_phs_capacity = pypsa_network.storage_units.groupby(
        "carrier"
    ).p_nom_opt.sum()
    df_pypsa_capacity = pd.concat(
        [
            pypsa_network.generators.groupby("carrier").p_nom_opt.sum(),
            df_pypsa_hydro_phs_capacity,
        ]
    )
    df_pypsa_capacity.loc["wind"] = df_pypsa_capacity.loc[
        ["offwind-ac", "offwind-dc", "onwind"]
    ].sum()
    ...
    eia_reference.loc["other biomass"] = eia_reference.loc[
        ["other biomass", "wood and wood-derived fuels"]
    ].sum()
    ...
    df_eia_capacity = df_eia_capacity.drop(
        [
            ...
            "estimated total photovoltaic",
            "other gases",
        ]
    )
    df_eia_capacity = df_eia_capacity.iloc[:-1]
    df_eia_capacity.loc["solar", "Generator Nameplate Capacity"] = df_eia_capacity.loc[
        "solar", "Net Summer Capacity"
    ]
    ...
    plt.savefig(
        pathlib.Path(plot_base_path, "countrywide_installed_capacity.png"), dpi=800
    )
...
if __name__ == "__main__":
    ...
    plot_path = pathlib.Path(default_path, snakemake.params.plot_directory)
    ...
    if snakemake.params.plot_state_by_state_comparison:
        plot_capacity_state_by_state_comparison(
            network_pypsa_earth_df,
            eia_state_temporal_installed_capacity_df,
            eia_raw_df,
            year_for_comparison,
    ...
    if snakemake.params.plot_spatial_representation:
        plot_capacity_spatial_representation(
            network_pypsa_earth_df,
            "capacity",
            snakemake.params.state_to_omit,
            plot_path,
            gadm_path,
        )
