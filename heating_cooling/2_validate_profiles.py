...
def convert_heat_demand2(ds, threshold, a, constant, hour_shift):
    ...
    dT_radiation = dT_radiation.assign_coords(
        time=(
            dT_radiation.coords["time"] + np.timedelta64(dt.timedelta(hours=hour_shift))
        )
    )
